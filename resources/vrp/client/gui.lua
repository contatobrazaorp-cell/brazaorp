...
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADBLOCK (CANCEL BLOCK CONTROLS)
-----------------------------------------------------------------------------------------------------------------------------------------
CreateThread(function()
	while true do
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADPHONE (BLOCK CONTROLS WHEN PHONE OPEN)
-----------------------------------------------------------------------------------------------------------------------------------------
CreateThread(function()
	while true do
		local TimeDistance = 999
		...
		if LocalPlayer["state"]["Active"] and (exports["horizonte_phone"]:IsOpen() or AnimVars[3]) then
			TimeDistance = 1
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- CANCEL (F6) - FIX: NÃO BLOQUEIA QUANDO Cancel=true + LIMPA ANIM/OBJ
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("Cancel",function()
	local Ped = PlayerPedId()

	if LocalPlayer["state"]["Active"]
		and GetGameTimer() >= Button
		and not IsPauseMenuActive()
		and not LocalPlayer["state"]["Handcuff"]
		and not exports["horizonte_phone"]:IsOpen()
...
RegisterCommand("Engine",function()
	local Ped = PlayerPedId()
	if LocalPlayer["state"]["Active"] and GetGameTimer() >= Button and not IsPauseMenuActive() and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not exports["horizonte_phone"]:IsOpen() and GetEntityHealth(Ped) > 100 and not LocalPlayer["state"]["Cancel"] and not IsPedReloading(Ped) then
		Button = GetGameTimer() + 500
		...
		if Vehicle and Vehicle ~= 0 and GetPedInVehicleSeat(Vehicle,-1) == Ped then
			...
			SetVehicleUndriveable(Vehicle,Running)
		end
	end
end)
...
RegisterCommand("NumBinds",function(source,Message)
	local Ped = PlayerPedId()
	if Message[1] and (Binded[Message[1]] or Message[1] == "0") and LocalPlayer["state"]["Active"] and GetGameTimer() >= Button and not IsPauseMenuActive() and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and not exports["horizonte_phone"]:IsOpen() and GetEntityHealth(Ped) > 100 and not LocalPlayer["state"]["Cancel"] and not IsPedReloading(Ped) and not LocalPlayer["state"]["Handcuff"] and not IsPedInAnyVehicle(Ped) and not IsPedArmed(Ped,7) and not IsPedSwimming(Ped) then
		Button = GetGameTimer() + 500
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- BINDS (ARRUMADO: TAVA QUEBRADO NO SEU ARQUIVO)
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("Binds",function(source,Message)
	if Message[1] and Message[2] and parseInt(Message[1]) >= 1 and parseInt(Message[1]) <= 9 then
		...
		TriggerEvent("Notify","Animações","A animação <b>"..Message[2].."</b> foi salva na tecla <b>"..Message[1].."</b>.","verde",5000)
	end
end)

-----------------------------------------------------------------------------------------------------------------------------------------
-- LOCK (FIX: PEGA VEÍCULO CERTO + NET ID CERTO)
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("Lock",function()
	local Ped = PlayerPedId()
	if LocalPlayer["state"]["Active"] and not LocalPlayer["state"]["BlockLocked"] and GetGameTimer() >= Button and not IsPauseMenuActive() and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not exports["horizonte_phone"]:IsOpen() and GetEntityHealth(Ped) > 100 and not LocalPlayer["state"]["Cancel"] and not IsPedReloading(Ped) then
		Button = GetGameTimer() + 500

		-- dentro do veículo
		local Vehicle = GetVehiclePedIsIn(Ped,false)

		-- se não estiver, tenta pegar o mais próximo
		if not Vehicle or Vehicle == 0 then
			Vehicle = tvRP.VehicleList and (select(1, tvRP.VehicleList())) or 0
		end
...
function tvRP.PersistentBlock(Item,Animation)
	...
	if not Persistent then
		Persistent = { ["Item"] = Item, ["Anim"] = Animation }
		TriggerEvent("emotes",Animation)
	elseif Persistent and Item ~= Persistent["Item"] then
		PersistentList[#PersistentList + 1] = { ["Item"] = Item, ["Anim"] = Animation }
	end
end
