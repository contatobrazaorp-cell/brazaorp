...
function Creative.Code(Name)
    ...
    local ConsultCodes = exports.oxmysql:single_async(
        "SELECT * FROM codes_creative WHERE Code = ? LIMIT 1",
        { Name }
    )
    ...
    -- Já resgatou?
    local Already = exports.oxmysql:single_async(
        "SELECT 1 FROM codes_creative_redeemd WHERE Code = ? AND Passport = ? LIMIT 1",
        { Name, Passport }
    )
    ...
    -------------------------------------------------------------------------
    local Rewards = {}
    if ConsultCodes.Rewards and ConsultCodes.Rewards ~= "" then
...
-------------------------------------------------------------------------
-- GIVE VEHICLE (MESMO PADRÃO DA BASE: vehicles/addVehicles)
-------------------------------------------------------------------------
local function GiveVehicle(model)
    model = tostring(model or ""):gsub("^%s+", ""):gsub("%s+$", "")
    if model == "" then return end

    -- se sua base salva em lowercase, deixa assim:
    -- model = model:lower()

    -- checa se já tem
    local CheckVehicle = vRP.SingleQuery("vehicles/selectVehicles",{ Passport = Passport, Vehicle = model })
    if CheckVehicle and not CheckVehicle.Block then
        TriggerClientEvent("pause:Notify", source, "Aviso", "Você já possui esse veículo.", "amarelo")
        return
    end
    ...
    TriggerClientEvent("pause:Notify", source, "Sucesso", "Veículo <b>"..VehicleName(model).."</b> recebido.", "verde")
end

    -------------------------------------------------------------------------
    -- APPLY REWARDS (aceita array ou map)
    -------------------------------------------------------------------------
    if type(Rewards) == "table" then
        if #Rewards > 0 then
            for _, r in ipairs(Rewards) do
                if type(r) == "table" then
                    local item   = r.Item or r.item
                    local amount = r.Amount or r.amount

                    if item == "vehicle" then
                        GiveVehicle(r.Model or r.model)
                    else
    ...
    -------------------------------------------------------------------------
    -- REGISTRA RESGATE (só agora)
    -------------------------------------------------------------------------
    exports.oxmysql:insert_async(
        "INSERT INTO codes_creative_redeemd (Code,Passport,RedeemdAt) VALUES (?,?,?)",
        { Name, Passport, os.time() }
    )

    exports.oxmysql:update_async(
        "UPDATE codes_creative SET Used = Used + 1 WHERE Code = ?",
        { Name }
    )
