...
AddEventHandler("mdt:Open",function(Group)
  ...
  -- Se abrirem como "Policia", converte pra corporação real (pra pegar Hierarchy certo)
  if Group == "Policia" then
    for _,v in pairs({ "PHZ","CIVIL","PRH","TATICA","EXERCITO" }) do
      if vRP.HasPermission(Passport,v) then
        Group = v
        break
      end
    end
    if Group == "Policia" then return end
  end

  Permission[Passport] = vRP.HasPermission(Passport, Group) and Group or nil
