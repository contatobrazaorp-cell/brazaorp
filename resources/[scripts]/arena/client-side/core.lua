...
local function Notify(msg)
	-- tenta notify do Creative/vrp se existir
	if exports and exports["notify"] then
		pcall(function()
			exports["notify"]:Notify("amarelo",msg,5000)
		end)
		return
	end
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- COMANDO PRA SAIR DA ARENA
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("sairarena",function()
	if LocalPlayer["state"]["Arena"] then
		TriggerEvent("arena:Exit")
	else
		Notify("Você não está em nenhuma arena.")
	end
end)
...
AddEventHandler("arena:Enter",function(Number)
	...
	if not LocalPlayer["state"]["Arena"] and Zones[Number] and vSERVER.CheckEnter(Zones[Number]) then
		...
		Notify("Você entrou na Arena. Use /sairarena para sair.")
	end
end)
