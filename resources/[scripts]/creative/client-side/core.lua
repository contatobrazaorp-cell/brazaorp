...
local BLIPS = {
	...
	{ 225.49,-1539.85,29.30, 402, 3, "Central de Utensílios", 0.7 },

	-- =========================
	-- NOVOS PONTOS COMERCIAIS
	-- =========================

	{ 46.29,-1749.3,29.64,78,62,"Mercado Central",0.6 }, -- Mercado Central

	{ -578.69,-1075.3,22.31,408,73,"Restaurante",0.6 }, -- Restaurante

	{ -373.88,-139.87,38.69,402,62,"Mecânica Central",0.7 }, -- MECÂNICA

	{ -2777.9,-57.74,18.6,80,38,"Hospital Sul",0.7 }, -- HOSPITAL SUL

	{ 1201.27,-1459.25,34.91,436,1,"Bombeiros",0.7 }, -- BOMBEIROS

	{ 2534.63,-365.49,92.99,60,49,"Tática",0.7 }, -- TÁTICA

	{ -2279.52,340.69,174.65,60,29,"PHZ",0.7 }, -- PHZ

	{ -2191.63,3109.1,32.81,110,17,"Exército",0.8 }, -- EXÉRCITO

	{ -453.62,6002.83,31.27,60,3,"PRF",0.7 }, -- PRF

	{ -787.74,-2325.75,14.86,60,26,"Polícia Civil",0.7 }, -- CÍVIL
...
local AMBIENT = {
	...
	-- densidade (0.0 a 2.0)
	VEHICLE_DENSITY = 0.55,
	RANDOM_VEHICLE_DENSITY = 0.55,
	PARKED_VEHICLE_DENSITY = 0.35,
	SCENARIO_PED_DENSITY = 0.65,
	PED_DENSITY = 0.65,
  
	-- quer polícia NPC / estrelas?
	DISABLE_RANDOM_COPS = true,
	DISABLE_WANTED = true,
  
	-- tráfego whitelist (opcional): suprime modelos indesejados
	TRAFFIC_ALLOWED = {
	  "sultan", "blista", "kuruma", "tailgater", "buffalo"
	},
	TRAFFIC_BLOCKED = { -- exemplos (tanque/jatos/bus etc.)
	  "rhino", "lazer", "hydra", "bus", "coach"
	},
	APPLY_TRAFFIC_FILTER = true,
  
	-- BLACKLIST POR ÁREA (não nasce npc/tráfego quando o player está dentro)
	BLACKLIST_ZONES = {
	  { name = "pier",  coords = vec3(-1626.02,-1058.5,13.14), radius = 120.0, peds = true, vehicles = true, parked = true, scenario = true },
	  { name = "praca", coords = vec3(187.65,-951.53,29.3),    radius = 100.0, peds = true, vehicles = true, parked = true, scenario = true }
	}
  }
  
  local function GetBlacklistZone(playerCoords)
	for _,z in ipairs(AMBIENT.BLACKLIST_ZONES) do
	  if #(playerCoords - z.coords) <= z.radius then
		return z
	  end
	end
	return nil
  ...
  -----------------------------------------------------------------------------------------------------------------------------------------
  -- TRAFFIC FILTER (WHITELIST/BLOCKLIST)
  -----------------------------------------------------------------------------------------------------------------------------------------
  CreateThread(function()
	Wait(2000)
  
	if not AMBIENT.APPLY_TRAFFIC_FILTER then return end
  
	-- libera os permitidos
	for _,model in ipairs(AMBIENT.TRAFFIC_ALLOWED) do
	  SetVehicleModelIsSuppressed(GetHashKey(model), false)
	end
  
	-- bloqueia os indesejados
	for _,model in ipairs(AMBIENT.TRAFFIC_BLOCKED) do
	  SetVehicleModelIsSuppressed(GetHashKey(model), true)
	end
  end)
  
  -----------------------------------------------------------------------------------------------------------------------------------------
  -- THREAD: CONTROLES / HUD / AMBIENTE
  -----------------------------------------------------------------------------------------------------------------------------------------
  CreateThread(function()
	...
	while true do
	  ...
	  if IsPedInAnyVehicle(ped) then
		DisableControlAction(0, 345, true) -- disable "duck" no carro (exemplo)
  
		local veh = GetVehiclePedIsUsing(ped)
  
		-- evita bug de trocar de banco / animações estranhas
		if not GetPedConfigFlag(ped, 184, true) then
		  SetPedConfigFlag(ped, 184, true)
		end
  
		-- força o jogador pro banco do motorista caso bugue uma task
		if GetPedInVehicleSeat(veh, -1) == ped and GetIsTaskActive(ped, 165) then
		  SetPedIntoVehicle(ped, veh, -1)
		end
  
		-- rappel heli (mantive sua lógica)
		if IsPedInAnyHeli(ped) and IsControlJustPressed(1,154) and not IsAnyPedRappellingFromHeli(veh) then
	  ...
	  SetAllVehicleGeneratorsActive() -- mantém geradores (spawn padrão do jogo)
  
	  CancelCurrentPoliceReport()
	  BlockWeaponWheelThisFrame()
  
	  if AMBIENT.DISABLE_RANDOM_COPS then
	  ...
	  -- =========================
	  -- POPULAÇÃO (NPC DO GTA) + BLACKLIST POR ZONA
	  -- =========================
	  if AMBIENT.ENABLE then
		local pCoords = GetEntityCoords(ped)
		local zone = GetBlacklistZone(pCoords)
  
		if zone then
		  -- dentro da zona: zera só o que a zona manda
		  SetVehicleDensityMultiplierThisFrame(zone.vehicles and 0.0 or AMBIENT.VEHICLE_DENSITY)
		  SetRandomVehicleDensityMultiplierThisFrame(zone.vehicles and 0.0 or AMBIENT.RANDOM_VEHICLE_DENSITY)
		  SetParkedVehicleDensityMultiplierThisFrame(zone.parked and 0.0 or AMBIENT.PARKED_VEHICLE_DENSITY)
  
		  local scen = (zone.scenario and 0.0 or AMBIENT.SCENARIO_PED_DENSITY)
		  SetScenarioPedDensityMultiplierThisFrame(scen, scen)
  
		  SetPedDensityMultiplierThisFrame(zone.peds and 0.0 or AMBIENT.PED_DENSITY)
		else
		  -- fora da zona: normal
		  SetVehicleDensityMultiplierThisFrame(AMBIENT.VEHICLE_DENSITY)
		  SetRandomVehicleDensityMultiplierThisFrame(AMBIENT.RANDOM_VEHICLE_DENSITY)
		  SetParkedVehicleDensityMultiplierThisFrame(AMBIENT.PARKED_VEHICLE_DENSITY)
		  SetScenarioPedDensityMultiplierThisFrame(AMBIENT.SCENARIO_PED_DENSITY, AMBIENT.SCENARIO_PED_DENSITY)
		  SetPedDensityMultiplierThisFrame(AMBIENT.PED_DENSITY)
		end
	  else
		-- se quiser “matar” o mundo, deixe ENABLE=false
	  ...
	  if AMBIENT.DISABLE_WANTED then
...
CreateThread(function()
	...
	for _,v in pairs(IPL_LIST) do
		...
		if v.Props then
			for _,idx in pairs(v.Props) do
				EnableInteriorProp(interior, idx)
			end
		end
	...
	for _,b in ipairs(BLIPS) do
		local blip = AddBlipForCoord(b[1], b[2], b[3])
		SetBlipSprite(blip, b[4])
		SetBlipDisplay(blip, 4)
		SetBlipAsShortRange(blip, true)
		SetBlipColour(blip, b[5])
		SetBlipScale(blip, b[7])

		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString(b[6])
	...
	while true do
		...
		if not IsPedInAnyVehicle(ped) then
			...
			for i = 1, #TELEPORT do
				if #(coords - TELEPORT[i][1]) <= 1.0 then
					timeDistance = 1

					if IsControlJustPressed(1,38) then
						SetEntityCoords(ped, TELEPORT[i][2])
					end
				end
			end
		end
...
local AMBIENT = {
	...
	-- BLACKLIST POR ÁREA (quando o player está dentro, não nasce npc/tráfego)
	BLACKLIST_ZONES = {
	  { name = "pier",  coords = vec3(-1626.02,-1058.5,13.14), radius = 120.0, peds = true, vehicles = true, parked = true, scenario = true },
	  { name = "praca", coords = vec3(187.65,-951.53,29.3),    radius = 100.0, peds = true, vehicles = true, parked = true, scenario = true }
	}
