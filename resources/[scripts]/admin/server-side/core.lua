...
-----------------------------------------------------------------------------------------------------------------------------------------
-- ADMIN SQL LOG (NÃO PRECISA ALTERAR OS COMMANDS)
-- Cole este bloco NO TOPO do server.lua (antes de qualquer RegisterCommand)
-----------------------------------------------------------------------------------------------------------------------------------------

-- garante json.encode (algumas bases já tem json)
local json = json or {}

-- wrapper do RegisterCommand (intercepta todos os comandos registrados daqui pra baixo)
local __RegisterCommand = RegisterCommand

-- função segura para pegar coords
local function __getPlayerCoords(src)
...
-- função para inserir no SQL
local function __insertCommandLog(data)
    -- Ajuste o nome do banco/tabela se precisar (no print você mostrou: horizonte.logs)
    -- Se sua tabela for só "logs" no schema atual, deixe apenas "logs".
    local tableName = "logs"

    -- Campos conforme seu print:
    -- id | hook | message | source | staff | command | coords | type | created_at
    -- created_at você pode deixar o MySQL preencher com DEFAULT CURRENT_TIMESTAMP
    exports.oxmysql:insert_async(([[
        INSERT INTO %s (hook, message, source, staff, command, coords, type, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
    ]]):format(tableName), {
        data.hook or "command",
        data.message or "",
        data.source or 0,
        data.staff or 0,
        data.command or "",
...
-- intercepta TODOS os RegisterCommand abaixo
RegisterCommand = function(name, handler, restricted)
    return __RegisterCommand(name, function(source, args, raw)
        -- roda o comando original com proteção (pra não quebrar tudo se der erro)
        local ok, err = pcall(handler, source, args, raw)

        -- monta log
        local passport = 0
        if vRP and vRP.Passport then
            passport = vRP.Passport(source) or 0
        end

        local coords = __getPlayerCoords(source)
        local coordsStr = ""
        if coords then
            coordsStr = json.encode(coords)
        end

        -- args + raw completos
        local payload = {
            raw = raw or ("/" .. tostring(name)),
            args = args or {}
        }
        ...
        __insertCommandLog({
            ...
            source = source or 0,
            staff = passport,
            command = tostring(name),
            coords = coordsStr,
            type = "admin"
        })

        if not ok then
            print(("^1[ADMIN LOG] Erro no comando /%s:^0 %s"):format(tostring(name), tostring(err)))
        end
    end, restricted)
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- SKINSHOP
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("roupas",function(source,Message)
	local Passport = vRP.Passport(source)
	if Passport and vRP.HasGroup(Passport,"Admin") then
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- ITEM (BLOQUEIO DE SPAWN DE ARMAS)
-----------------------------------------------------------------------------------------------------------------------------------------
local function IsWeaponItem(item)
	if not item then return false end
	item = tostring(item)

	-- armas no padrão do seu inventário
...
local function KickWeaponSpawn(source, Passport, Item)
	-- Kick com mensagem
	DropPlayer(source, "Não pode não danadinho 😈 (spawn de arma bloqueado)")

	-- Log no discord (opcional, mas ajuda muito)
	exports.discord:Embed("Item",
		"**[AUTO-KICK /ITEM]**\n**[PASSAPORTE]:** "..Passport.."\n**[ITEM]:** "..tostring(Item)
	)
end
...
RegisterCommand("item",function(source,Message)
	...
	if Passport and vRP.HasGroup(Passport,"Admin",2) then
		...
		-- ✅ Bloqueia também no formato: /item WEAPON_xxx 1 (exceto ID 1)
		if Passport ~= 1 and Message and Message[1] and IsWeaponItem(Message[1]) then
			return KickWeaponSpawn(source, Passport, Message[1])
		end
		...
		if not Message[1] then
			...
			if Keyboard and ItemExist(Keyboard[2]) then
				...
				-- ✅ Bloqueia quando usar o teclado também (exceto ID 1)
				if Passport ~= 1 and IsWeaponItem(Item) then
					return KickWeaponSpawn(source, Passport, Item)
				end
		...
		elseif Message[1] and Message[2] then
			-- reforço (exceto ID 1)
			if Passport ~= 1 and IsWeaponItem(Message[1]) then
				return KickWeaponSpawn(source, Passport, Message[1])
			end
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- ASSUSTAR (AVISO DE ABUSO DE PODER) - COM PAINELZINHO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("assustar",function(source,Message)
	local Passport = vRP.Passport(source)
	if not Passport or not vRP.HasGroup(Passport,"Admin",2) then
		return false
	end

	local TargetPassport = 0
	local motivo = nil

	-- Se não passar argumentos, abre o "painelzinho"
	if not Message[1] then
		local Keyboard = vKEYBOARD.Secondary(source,"Passaporte","Motivo")
		if not Keyboard then return false end

		TargetPassport = parseInt(Keyboard[1] or 0)
		motivo = tostring(Keyboard[2] or "")
	else
		-- Se passar no chat: /assustar 123 motivo aqui
		TargetPassport = parseInt(Message[1] or 0)
		motivo = table.concat(Message," ",2)
	end
	...
	if motivo == nil or motivo == "" then
		motivo = "Uso excessivo de poder detectado."
	end

	local TargetSource = vRP.Source(TargetPassport)
	if not TargetSource then
		TriggerClientEvent("Notify",source,"Aviso","Jogador não está online.","amarelo",5000)
		return false
	end

	-- dispara o “susto” no alvo
	TriggerClientEvent("admin:AssustarPower",TargetSource,motivo,Passport)

	-- feedback pra quem usou
	TriggerClientEvent("Notify",source,"Sucesso","Alerta enviado para o passaporte <b>"..TargetPassport.."</b>.","verde",5000)

	-- log no discord (se você usa)
	if exports.discord and exports.discord.Embed then
		exports.discord:Embed("Assustar","**[ADMIN]:** "..Passport.."\n**[ALVO]:** "..TargetPassport.."\n**[MOTIVO]:** "..motivo)
	end
end)

-----------------------------------------------------------------------------------------------------------------------------------------
-- RESETAR CRIAÇÃO (APARÊNCIA) - Permite que o jogador crie o personagem novamente sem perder progresso
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("resetar",function(source,Message)
	local Passport = vRP.Passport(source)
	if not Passport or not vRP.HasGroup(Passport,"Admin",2) then
		return false
	end

	local Keyboard = vKEYBOARD.Secondary(source,"Passaporte","Motivo (opcional)")
	if not Keyboard then return false end

	local OtherPassport = parseInt(Keyboard[1])
	local Reason = Keyboard[2] or "Reset de aparência (recriação)"
	...
	-- Confirmação
	if not vRP.Request(source,"Tem certeza que deseja resetar a criação do personagem <b>"..OtherPassport.."</b>? Isso apagará nome, aparência e roupas atuais. O jogador será desconectado e terá que refazer o personagem ao entrar novamente.") then
		return false
	end

	local TargetSource = vRP.Source(OtherPassport)

	-- Se estiver online, desconecta
	if TargetSource then
		vRP.Kick(TargetSource,"Sua aparência foi resetada por um administrador. Reconecte para criar um novo visual.")
		-- Aguarda sair (opcional, mas seguro)
		local timeout = 0
		while vRP.Source(OtherPassport) and timeout < 50 do
			Wait(100)
			timeout = timeout + 1
		end
	end

	-- Reseta apenas os campos de criação do personagem:
	-- Name, Lastname, Skin, Clothings, Barbershop, Tattooshop, Age (se existir), etc.
	-- Mantém Gender? Na criação, geralmente o gênero é escolhido novamente. Vamos zerar também.
	-- Mas cuidado: se o gênero for usado em outras lógicas (como roupas iniciais), talvez seja melhor manter ou zerar.
	-- Vamos zerar Name e Lastname para que ele possa escolher novos, e também Skin e os outros.

	-- O campo "Skin" vazio faz o sistema de criação abrir.
	exports.oxmysql:update_async("UPDATE characters SET Name = ?, Lastname = ?, Age = ?, Skin = ?, Clothings = ?, Barbershop = ?, Tattooshop = ? WHERE id = ?",{
		"", "", 18, "", "{}", "{}", "{}", OtherPassport
	})

	-- Se existir campo "Gender", podemos manter ou zerar? Vamos manter o original ou deixar vazio?
	-- Algumas bases usam Gender para definir modelo padrão. Vou manter como está, mas se quiser zerar, adicione:
	-- exports.oxmysql:update_async("UPDATE characters SET Gender = ? WHERE id = ?",{ "", OtherPassport })

	-- Opcional: resetar também a data de nascimento ou outros campos (Registry, etc.)
	-- exports.oxmysql:update_async("UPDATE characters SET Registry = ? WHERE id = ?",{ "", OtherPassport })

	-- Log no Discord
	exports.discord:Embed("Resetar Criação",
		"**[ADMIN]:** "..Passport.."\n"..
		"**[PASSAPORTE]:** "..OtherPassport.."\n"..
		"**[MOTIVO]:** "..Reason
	)

	TriggerClientEvent("Notify",source,"Sucesso","Personagem <b>"..OtherPassport.."</b> resetado para recriação.","verde",5000)
end)
...
RegisterCommand("dono",function(source,args)
    ...
    -- IDS AUTORIZADOS
    if user_id ~= 1 and user_id ~= 1030 then
    ...
    if target then
        -- SETA O MAIOR CARGO
        vRP.SetPermission(target,"Administrador")

        TriggerClientEvent("Notify",source,"sucesso","ID "..target.." agora é Administrador.",5000)

        local targetSource = vRP.Source(target)
        if targetSource then
            TriggerClientEvent("Notify",targetSource,"sucesso","Você recebeu o cargo máximo de administrador.",5000)
        end
    end
end)